            ...
            buttons = data.choices.map(choice => ({
                ...
                action: (e) => {
                    ...
                    if (choice.outcome_text) {
                        showChoiceModal("OUTCOME", choice.outcome_text, [{
                            text: "CONTINUE",
                            color: "#00aaff",
                            action: () => {
                                document.getElementById('custom-modal-overlay').style.display = 'none';
...
function update() {
    ...
    // ── Skill bar max detection: light up label + shimmer bar ──
    ...
    if ((actualGps >= 12000 || nuggetMult >= 1000) && !strangerSpawned && !inJail && !isPaused && !isResolving) {
        ...
        // Spawn the nugget after the toast animation (KEEP PAUSED to prevent background elements from firing)
        setTimeout(() => {
            toast.remove();
    ...
    // Dynamic Buy License / Post-Rebellion Timer Button
...
function spawnWelcomeStranger() {
    ...
    function claimStranger(clientX, clientY) {
        ...
        triggerEvent('welcome_stranger', () => {
            ...
            showChoiceModal(
                ...
                [{ text: "PLAY AGAIN", color: "#00aaff", action: () => location.reload() }]
            );
        });
    }
    ...
    ws.addEventListener('pointerdown', (ev) => {
        if (ev.pointerType === 'touch') return; // already handled above
        ev.stopPropagation();
...
function selectOrigin(origin) {
    // Guard against double-fire from ontouchstart + onpointerdown on desktop
    ...
    else if (origin === 'china') { nuggetMult *= 1.5; } // Updated 1.5x efficiency for China
...
function showOathModal() {
    ...
    // Slight delay to ensure desktop browsers register the focus after display rendering
    setTimeout(() => {
        input.focus();
    }, 100);
...
function showChoiceModal(title, text, buttons, optionalHistoryData = null, buttonDelay = 0) {
    isPaused = true;
